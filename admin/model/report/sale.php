...
class ModelReportSale extends Model {
	public function getTotalSales($data = array()) {
		...
		if(isset($this->session->data['storeid']) && $this->session->data['storeid']>0){
			$sql .= " AND store_id = '" . (int)$this->session->data['storeid'] . "'";
		}
	...
	public function getTotalOrdersByCountry() {
		$where="";
		if(isset($this->session->data['storeid']) && $this->session->data['storeid']>0){
			$where .= " AND o.store_id = '" . (int)$this->session->data['storeid'] . "'";
		}

		$query = $this->db->query("SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "country` c ON (o.payment_country_id = c.country_id) WHERE o.order_status_id > '0' $where GROUP BY o.payment_country_id");
		return $query->rows;
	}
	...
	public function getTotalOrdersByDay() {
		...
		$where = "";
		if(isset($this->session->data['storeid']) && $this->session->data['storeid']>0){
			$where .= " AND store_id = '" . (int)$this->session->data['storeid'] . "'";
		}

		$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) = DATE(NOW()) $where GROUP BY HOUR(date_added) ORDER BY date_added ASC");
	...
	public function getTotalOrdersByWeek() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') $where GROUP BY DAYNAME(date_added)");
	...
	public function getTotalOrdersByMonth() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' $where GROUP BY DATE(date_added)");
	...
	public function getTotalOrdersByYear() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND YEAR(date_added) = YEAR(NOW()) $where GROUP BY MONTH(date_added)");

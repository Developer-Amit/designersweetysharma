...
class ModelReportCustomer extends Model {
	public function getTotalCustomersByDay() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "customer` WHERE DATE(date_added) = DATE(NOW()) GROUP BY HOUR(date_added) $where ORDER BY date_added ASC");
	...
	public function getTotalCustomersByWeek() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "customer` WHERE DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') $where GROUP BY DAYNAME(date_added)");
	...
	public function getTotalCustomersByMonth() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "customer` WHERE DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' $where GROUP BY DATE(date_added)");
	...
	public function getTotalCustomersByYear() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "customer` WHERE YEAR(date_added) = YEAR(NOW()) $where GROUP BY MONTH(date_added)");

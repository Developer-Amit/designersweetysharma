<?php
class ModelNilkanthinfoNilkanthInfoTestimonial extends Model {

	public function addTestimonial($data) {
	
		$i=array_keys($data['testimonial_description']);
		$j=$i[0];
	

		$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial SET name='".$this->db->escape($data['testimonial_description'][$j]['name'])."', city = '".$this->db->escape($data['testimonial_description'][$j]['city'])."', status = '" . (int)$data['status'] . "',rating = '".(int)$data['testimonial_description'][$j]['rating'] . "',date_added = '" . $this->db->escape($data['testimonial_description'][$j]['date_added']) . "',email='" . $this->db->escape($data['testimonial_description'][$j]['email']) . "'");

		$testimonial_id = $this->db->getLastId(); 
		
		if (isset($data['testimonial_description'][$j]['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "testimonial SET image = '" . $this->db->escape($data['testimonial_description'][$j]['image']) . "' WHERE testimonial_id = '" . (int)$testimonial_id . "'");
		}
	...
	public function editTestimonial($testimonial_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "testimonial SET name='".$this->db->escape($data['testimonial_description'][$j]['name'])."', city = '".$this->db->escape($data['testimonial_description'][$j]['city'])."', status = '" . (int)$data['status'] . "',date_added = '".$this->db->escape($data['testimonial_description'][$j]['date_added']). "',rating = '".(int)$data['testimonial_description'][$j]['rating']."',email='". $this->db->escape($data['testimonial_description'][$j]['email']) ."' WHERE testimonial_id = '" . (int)$testimonial_id . "'");
	...
	public function getTestimonialDescriptions($testimonial_id) {
		...
		$query = $this->db->query("SELECT td.*,t.* FROM " . DB_PREFIX . "testimonial_description as td," . DB_PREFIX . "testimonial as t WHERE t.testimonial_id=td.testimonial_id AND td.testimonial_id = '" . (int)$testimonial_id . "'");

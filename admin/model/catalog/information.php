...
class ModelCatalogInformation extends Model {
	...
	public function getInformations($data = array()) {
		if ($data) {
			$storesql = "";
			if (!empty($data['storeid']) && $data['storeid']>0) {
				$storesql .= " INNER JOIN " . DB_PREFIX . "information_to_store ins ON ins.information_id=i.information_id";
			}

			$sql = "SELECT * FROM " . DB_PREFIX . "information i $storesql LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "'";

			if (!empty($data['storeid']) && $data['storeid']>0) {
				$sql .= " AND ins.store_id = " . $this->db->escape($data['storeid']);
			}
	...
	public function getTotalInformations() {
		...
		$storesql = "";
		if($storeid>0) {
			$storesql .= " INNER JOIN " . DB_PREFIX . "information_to_store ins ON ins.information_id=i.information_id WHERE ins.store_id = " . $storeid;
		}

		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "information i $storesql");

            ...
            <tbody>
              ...
              <tr id="module-row<?php echo $module_row; ?>">
                ...
                <td class="text-left"><select name="layout_module[<?php echo $module_row; ?>][position]" class="form-control">
                    <?php 
					if( strpos($layout_module['code'],'nilkanthinfo_latest_blog') !== false){
					}else{
                    ...
                    <?php } ?>
					<!--Nilaknthinfo CMS content header footer options -->
					<?php 
					
					if( strpos($layout_module['code'],'nilkanthinfo_cms')!== false){
						
						 if ($layout_module['position'] == 'content_footer') { ?>
						...
						<?php } 
						 if ($layout_module['position'] == 'content_foter1') { ?>
						<option value="content_foter1" selected="selected"><?php echo $text_content_footer1; ?></option>
						<?php } else { ?>
						<option value="content_foter1"><?php echo $text_content_footer1; ?></option>
						<?php } 
						 if ($layout_module['position'] == 'content_foter2') { ?>
						<option value="content_foter2" selected="selected"><?php echo $text_content_footer2; ?></option>
						<?php } else { ?>
						<option value="content_foter2"><?php echo $text_content_footer2; ?></option>
						<?php } 
						 if ($layout_module['position'] == 'content_foter3') { ?>
						<option value="content_foter3" selected="selected"><?php echo $text_content_footer3; ?></option>
						<?php } else { ?>
						<option value="content_foter3"><?php echo $text_content_footer3; ?></option>
						<?php } 
					...
					<!--Nilaknthinfo Newsletter content footer options -->
					<?php 
					
					if( strpos($layout_module['code'],'nilkanthinfo_newsletter')!== false){
...
//function to add option for Nilkanthinfo CMS
var count=0;
var count1=0;
function addpos(row)
{
	...
	if ($('select[name="layout_module['+row+'][code]"]').val().indexOf("nilkanthinfo_cms")>=0){
		
		$('select[name="layout_module['+row+'][position]"] option[value="content_footer"]').remove( );
		$('select[name="layout_module['+row+'][position]"] option[value="content_foter1"]').remove( );
		$('select[name="layout_module['+row+'][position]"] option[value="content_foter2"]').remove( );
		$('select[name="layout_module['+row+'][position]"] option[value="content_foter3"]').remove( );
		
		
		$('select[name="layout_module['+row+'][position]"]').append( ' <option value="content_footer"><?php echo $text_content_footer; ?></option><option value="content_foter1"><?php echo $text_content_footer1; ?></option><option value="content_foter2"><?php echo $text_content_footer2; ?></option><option value="content_foter3"><?php echo $text_content_footer3; ?></option>');
		count++;
	}
	else if(count!=0)
	{
			$('select[name="layout_module['+row+'][position]"] option[value="content_header"]').remove( );
				$('select[name="layout_module['+row+'][position]"] option[value="content_footer"]').remove( );
					$('select[name="layout_module['+row+'][position]"] option[value="content_foter1"]').remove( );
		$('select[name="layout_module['+row+'][position]"] option[value="content_foter2"]').remove( );
		$('select[name="layout_module['+row+'][position]"] option[value="content_foter3"]').remove( );
	...
	if ($('select[name="layout_module['+row+'][code]"]').val().indexOf("nilkanthinfo_newsletter")>=0){
...
function removepos(row){
		if ($('select[name="layout_module['+row+'][code]"]').val().indexOf("nilkanthinfo_latest_blog")>=0){
				$('select[name="layout_module['+row+'][position]"] option[value="content_top"]').remove( );
				$('select[name="layout_module['+row+'][position]"] option[value="content_bottom"]').remove( );
		}
}
...
function addModule() {
	...
	html += '  <td class="text-left"><select onchange="addpos('+ module_row+');removepos('+ module_row+');" name="layout_module[' + module_row + '][code]" class="form-control">';
	<?php foreach ($extensions as $extension) { ?>    
	<?php if (!$extension['module']) { ?>

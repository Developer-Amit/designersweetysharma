...
class ControllerUserUser extends Controller {
	...
	protected function getForm() {
		$data['heading_title'] = $this->language->get('heading_title');
		require($_SERVER['DOCUMENT_ROOT'].'/designersweetysharma/admin/model/setting/store.php');
		$stores = new ModelSettingStore( $this->registry );
		$data['stores_data'] = $stores->getStores();

		$data['text_form'] = !isset($this->request->get['user_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		$data['text_enabled'] = $this->language->get('text_enabled');
		$data['text_disabled'] = $this->language->get('text_disabled');
		$data['text_selectstore'] = $this->language->get('text_selectstore');
		...
		if (isset($this->request->post['storeid'])) {
			$data['store'] = $this->request->post['storeid'];
		} elseif (!empty($user_info)) {
			$data['store'] = $user_info['storeid'];
		} else {
			$data['store'] = 0;
		}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen(trim($this->request->post['storeid'])) < 1)) {

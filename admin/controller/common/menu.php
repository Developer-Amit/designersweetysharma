...
class ControllerCommonMenu extends Controller {
	public function index() {
		...
		if(isset($this->session->data['storeid'])){
			$storeid = $this->session->data['storeid'];
		}else{
			$storeid = 0;
		}
		...
		$data['text_ni'] = $this->language->get('text_ni');
		
		if(in_array("nilkanthinfo_testimonial",$extensions))
		{
				$data['text_nilkanthinfo_testimonial'] = $this->language->get('text_nilkanthinfo_testimonial');	//menu option for nilkanthinfo_testimonial
				$data['nilkanthinfo_testimonial'] = $this->url->link('nilkanthinfo/nilkanthinfo_testimonial', 'token=' . $this->session->data['token'], 'SSL');		//Option link for nilkanthinfo_testimonial
		
		}
		else{
				$data['text_nilkanthinfo_testimonial'] = "";
				$data['nilkanthinfo_testimonial'] = "";
		}
		
		if(in_array("nilkanthinfo_newsletter",$extensions))
		{
				$data['text_nilkanthinfo_newsletter'] = $this->language->get('text_nilkanthinfo_newsletter');	// menu option for nilkanthinfo_newsletter
				$data['nilkanthinfo_newsletter'] = $this->url->link('nilkanthinfo/nilkanthinfo_newsletter', 'token=' . $this->session->data['token'], 'SSL');	//Option link for nilkanthinfo_newsletter
		}
		else{
				$data['text_nilkanthinfo_newsletter'] = "";
				$data['nilkanthinfo_newsletter'] = "";
		}
		
		
		
		if(in_array("nilkanthinfo_blog",$extensions))
		{
				$data['text_ni_blog'] = $this->language->get('text_ni_blog');
				$data['text_ni_blog_comment'] = $this->language->get('text_ni_blog_comment');
			
				$data['ni_blog'] = $this->url->link('nilkanthinfo/ni_blog', 'token=' . $this->session->data['token'], 'SSL');
				$data['ni_blog_comment'] = $this->url->link('nilkanthinfo/ni_blog_comment', 'token=' . $this->session->data['token'], 'SSL');			
		}
		else{
				$data['text_ni_blog'] = '';
				$data['text_ni_blog_comment'] = '';
				
				$data['ni_blog'] = '';		
				$data['ni_blog_comment'] = '';	
		}
		
		$data['text_nilkanthinfo_default'] = $this->language->get('text_nilkanthinfo_default');
		$data['NI_default'] = $this->url->link('nilkanthinfo/ni_setting', 'token=' . $this->session->data['token'], 'SSL');
		if (!$this->user->hasPermission('access', 'nilkanthinfo/ni_setting')) {
		$this->load->model('user/user_group');
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'nilkanthinfo/ni_setting' );
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'nilkanthinfo/ni_setting' );
		}

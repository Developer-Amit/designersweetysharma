...
class ModelPaymentPPExpress extends Model {
    ...
    public function convertAmount($amount){
		...
		return sprintf("%.2f",round($converted_amount));
    }
	...
	public function paymentRequestInfo() {
		...
		foreach ($this->cart->getProducts() as $item) {
           ...
           // $data['L_PAYMENTREQUEST_0_AMT' . $i] =   sprintf("%.2f",2.60);

			$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->convertAmount($item_price);
            $myprice = $myprice +  $this->convertAmount($item_price);
		...
		foreach ($total_data['totals'] as $total_row) {
			if (!in_array($total_row['code'], array('total', 'sub_total'))) {
				if ($total_row['value'] != 0) {
					...
					$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->convertAmount($this->currency->format($total_row['value'], $this->session->data['currency'], false, false));
					$data['L_PAYMENTREQUEST_0_QTY' . $i] = 1;
                    $shipping_cost  =  $shipping_cost + $this->convertAmount($this->currency->format($total_row['value'], $this->session->data['currency'], false, false));
        ...
        //$data['PAYMENTREQUEST_0_ITEMAMT'] = sprintf("%.2f",3.60);
		//$data['PAYMENTREQUEST_0_AMT'] = sprintf("%.2f",3.60);
		// $myamount = 0;
		// foreach ($this->cart->getProducts() as $item) {
		// // 	$myamount  = $myamount + $item_price = $this->currency->format($item['price'], $this->session->data['currency'], false, false);
		// }
         
		
  //       $data['PAYMENTREQUEST_0_ITEMAMTNEW'] = $myprice+$shipping_cost;
		// $data['PAYMENTREQUEST_0_ITEMAMT'] = $this->convertAmount(number_format($item_total, 2, '.', ''));
		// $data['PAYMENTREQUEST_0_AMT'] = $this->convertAmount(number_format($item_total, 2, '.', ''));
         
        $data['PAYMENTREQUEST_0_ITEMAMT'] = $myprice+$shipping_cost;
		$data['PAYMENTREQUEST_0_AMT'] = $myprice+$shipping_cost;

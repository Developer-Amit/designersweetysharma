...
class ModelAccountCustomer extends Model {
	...
	public function addCustomerByFB($data) {
		$customer_id = $this->customer->getId();
		$custname = explode(' ',$data['name']);
		$custcount = $this->getTotalCustomersByEmail($data['email']);
		if($custcount==0){
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($custname[0]) . "', lastname = '" . $this->db->escape($custname[1]) . "', email = '" . $this->db->escape($data['email']) . "', status = '1', customer_group_id = '1', approved = '1',password = '".md5($data['fbid'])."'");
			$customer_id = $this->db->getLastId();
		}else{
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($custname[0]) . "', lastname = '" . $this->db->escape($custname[1]) . "', email = '" . $this->db->escape($data['email']) . "', status = '1', customer_group_id = '1', approved = '1',password = '".md5($data['fbid'])."' WHERE email='".$this->db->escape($data['email'])."'");
		}
		return $customer_id;
	}
	...
	public function resetPassword($customer_id, $password) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($password)))) . "', code = '' WHERE customer_id = '" . $this->db->escape($customer_id) . "'");
	}

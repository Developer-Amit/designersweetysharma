...
class ControllerAccountRegister extends Controller {
	...
	private function validate() {
		...
		}else if ((utf8_strlen($this->request->post['telephone']) < 10) || (utf8_strlen($this->request->post['telephone']) > 10)) {
	...
	public function registerorloginwithfb() {
		// if ($this->customer->isLogged()) {
		// 	echo json_encode(array('status'=>'success','customer_id'=>$this->customer->customer_id));
		// }

		$this->load->model('account/customer');
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$customer_id = $this->model_account_customer->addCustomerByFB($this->request->post);

			// Clear any previous login attempts for unregistered accounts.
			$this->model_account_customer->deleteLoginAttempts($this->request->post['email']);

			$this->customer->login($this->request->post['email'], $this->request->post['fbid']);
